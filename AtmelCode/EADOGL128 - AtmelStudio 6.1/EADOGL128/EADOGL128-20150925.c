...
//Hilfsvariablen Zeitdarstellung und Zeitmessung
int timeXPos = 0;
int alarmXPos = 0;
...
//Eingabe
int rotary = 0;//rotary = -1 -> wurde nach links gedreht, +1 nach rechts gedreht, 0 nicht bewegt
bool btn_drehenc_pushed = false;
float btn_press_duration = 0;
bool light_status = OFF;
int light_sec_off = 0;//gibt Sekunde an, zu der das Licht wieder ausgeschaltet werden soll

//Menüsteuerung
bool menuOpen = false;
int old_selectedItem = 0;

int hourWidth = 0;//Breite der Zahl(en) auf dem Display, die Stunden angeben [in Pixeln]
int minuteWidth = 0;// --"--- Minuten --> wird genutzt um Zeitanzeige zu zentrieren
uint8_t herzSymb[32];
...
int main(void)
{
	...
	DDRC = 0xFF;//Ausgang
	DDRC &= ~(1<<PORTC0);//Taster Licht
	DDRC &= ~(1<<PORTC5);//Taster Drehencoder
//	PORTC = 0xff;
//	PORTB = 0xff;
	//DDRC |= 1;//NUR TEST 
	...
	while(1) {
		...
		if(btn_drehenc_pushed){
			...
			while((PINC & (1<<PINC5)) && (btn_press_duration<BTN_PRESS_LONG)){_delay_ms(50);}//PRÜFEN: KANN DELAY WEGGELASSEN WERDEN?
			//Zeitmessung beenden
			stop_btnPress();
			//falls länger als BTN_PRESS_LONG gedrückt wurde, Menü öffnen
			if(btn_press_duration > BTN_PRESS_LONG){
				//Symbole laden, die werden gleich in den Menüs gebraucht: 
				for(int i=0; i<32; i++){
				...
				//PRÜFEN: LICHT ANMACHEN? WECKER TOGGLEN!
			}
	...
	ISR(PCINT1_vect){//Drehencoder/Lichtschalter wurde gedrückt oder losgelassen
	...
	ISR(INT0_vect){//Drehencoder wurde gedreht
		if(light_status)//Licht-An-Dauer verlängern, falls Licht an ist
			light_sec_off = (seconds + LIGHT_ON_DURATION)%60;
		int A = (PIND & (1<<PIND2));
		int B = (PIND & (1<<PIND0));
		if((A && B) || ((!A)&&(!B))) //A=B
	...
	ISR(TIMER2_OVF_vect) {//Quarz-Overflow (1s vorbei)
		...
		//DAS HIER AN, FALLS timeAdvance = 1: if((timeAdvance != 137932) && increment ){ //nach 137932s ist Wecker 2s zu schnell
			seconds ++;
			if( seconds == 60 ) {
				timeUpdate++;
				seconds = 0;
...
void print_mainMenu(){
	...
	//Weckzeit
	int weckzeit_ASCII [8];
	convertString("Weckzeit", weckzeit_ASCII);
	print_ASCIIString(20, 3, weckzeit_ASCII, sizeof(weckzeit_ASCII)/sizeof(weckzeit_ASCII[0]));
	
	//Uhrzeit
	int uhrzeit_ASCII [7];
	convertString("Uhrzeit", uhrzeit_ASCII);
	print_ASCIIString(20, 18, uhrzeit_ASCII, sizeof(uhrzeit_ASCII)/sizeof(uhrzeit_ASCII[0]));
	
	//Datum
	int datum_ASCII [5];
	convertString("Datum", datum_ASCII);
	print_ASCIIString(20, 33, datum_ASCII, sizeof(datum_ASCII)/sizeof(datum_ASCII[0]));
	
	// . . . 
	print_letter(alphaHeight, pgm_read_byte(&alphaWidth[54]), 54, 20, 48);
	print_letter(alphaHeight, pgm_read_byte(&alphaWidth[54]), 54, 26, 48);
	print_letter(alphaHeight, pgm_read_byte(&alphaWidth[54]), 54, 32, 48);
...
int get_menuSelection(int numItems, int selectedItem){
	print_menuSelection(selectedItem);
	update_LCD();
	cli();
	rotary = 0;
	btn_drehenc_pushed = false;
...
void print_menuSelection(int item){
...
void configureTimerAndInterrupts() {
	...
	PCMSK1 = (1<<PCINT0);//löst Interrupt bei Drücken von Lichtschalter aus
}
...
int get_stringWidth(int string [], int length){
	int width = -1;
...
void goodNight() {
	while((btn_drehenc_pushed==false) && (rotary==0)){ //wenn er aufwacht, überprüfen ob es vom Drücken oder Loslassen des Schalters kommt
		SMCR |= (1<<SE);
		SMCR |= ((1<<SM1)|(1<<SM0));
